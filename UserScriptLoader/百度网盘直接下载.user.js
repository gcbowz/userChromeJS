...
// 获取当前域名
var QQ_LIXIAN = QQ_FENXIANG = VMALL = XUNLEI_KUAI = BAIDU_PAN = BAIDU_PAN_SHARE_DIRECTORY = BAIDU_PAN_SHARE_DIRECTORY_S = BAIDU_PAN_HOME = BAIDU_PAN_HOME_DIRECTORY = BAIDU_PAN_INIT = BAIDU_PAN_PLAY = YUN360_PAN = YUN360_PAN_HOME = false;
...
(NOWHOST = function() {
    ...
    switch (URL_OBJECT.host) {
        ...
        case 'yunpan.360.cn':
            YUN360_PAN = true;
            YUN360_PAN_HOME = URL_OBJECT.host == 'yunpan.360.cn' && (URL_OBJECT.directory == '/my/index/' || URL_OBJECT.directory == '/my/');
            break;
        default:
            break;
    }
...
var script_Update = function(name, version, regex, name_patch) {
    ...
    for (var i = scripts.length - 1; i >= 0; --i) {
        ...
        if (scriptRealName == name) {
            ...
            var isShow = parseInt(gm_win.localStorage.getItem(key)) <= parseInt(TIMESTAMP());
...
var BAIDU_PAN_FUNCTION = function(hashchange) {
    ...
    var _autod = function() {
        ...
        _.prototype = {
            _requestDownloadManager: function(_, A) {
                $.getScript("http://pan.baidu.com/res/static/js/ui/download_manager.js").done(_).fail(A);
            },
            _calculateMode: function(_) {
                //if (typeof _.length != "number" && _.size > disk.util.DownloadManager.SIZE_THRESHOLD) {
            ...
            _test0: function(_, C) {
                ...
                A.setMessage(disk.ui.Toast.MODE_LOADING, "\u521d\u59cb\u5316\u7ec4\u4ef6&hellip;");
                ...
                var B = function() {
                    ...
                    if (A == null) {
                        ...
                        if (E._mWatcher != null && typeof A.addVisibilityEventWatcher != "undefined") {
                    ...
                    D = function() {
                        ...
                        _.setMessage(disk.ui.Toast.MODE_FAILURE, "\u521d\u59cb\u5316\u7ec4\u4ef6\u5931\u8d25\uff0c\u8bf7\u7a0d\u540e\u91cd\u8bd5");
            ...
            straightforwardDownload: function(A, C) {
                _._warmupHTML();
                if (C === 1) {
                    ...
                    if (A.dlink && A.dlink.indexOf("&amp;") != -1) {
                        FileUtils._mDiskLog.send({
                            type: "downloadby_browser_link"
                        });
                    }
                } else {
            ...
            download: function(A, B) {
                if (!_.isPlatformCompatible() || !this._useAdvanceFeature()) {
                    this.straightforwardDownload(A, B);
                    return;
                }
                this._test0(A, B);
            },
    ...
    var _callback = function(md5listJSONobj) {
        ...
        mjQuery('div.time-col').each(function() {
            ...
            if (!is_dir && !is_other && md5listJSONobj[position] && md5listJSONobj[position].dlink) {
                var dlink = md5listJSONobj[position].dlink;
                urls += dlink ? dlink + '\n' : '';
                if (_self.find('span').length != 2) {
                    _self.find('span').prepend('<span style="cursor: pointer;"><a _position="' + position + '" class="down_normal_z" style="float:right; margin-top:5px;" href="' + dlink + '">直接下载</a></span>&nbsp;&nbsp;');
                } else {
    ...
    if (!hashchange) {
        ...
        if (buttom.length && gm_win.disk && gm_win.disk.util && gm_win.disk.util.ViewShareUtils && gm_win.disk.util.ViewShareUtils.viewShareData) {
            ...
            mjQuery.get(geturl, function(data) {
                ...
                var start_normal_down = buttom.find('b').html().replace(/[^(]+(\([^)]+\))/ig, "普通下载$1");
                mjQuery('#start_normal_down').find('b').html(start_normal_down);
                mjQuery('#shareqr').after('<br /><span>文件MD5:<input type="text" style="width:250px;" value="' + mjQuery.parseJSON(gm_win.disk.util.ViewShareUtils.viewShareData).md5 + '"></span>');
